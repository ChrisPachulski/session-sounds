...
use crate::herdr::{Herdr, Metadata};
use crate::state::{
    apply_status_observation, assign_sound, cleanup_pane, move_pane, reconcile_under_pressure,
    reshuffle_pane, Assignment, StateStore,
};
...
fn event(
    ...
    match event.kind.as_str() {
        ...
        "pane_moved" => {
            ...
            store
                .transaction(|state| {
                    move_pane(state, &previous, &new_pane_id, workspace.as_deref());
                    Ok(())
                })
                .map_err(|error| error.to_string())?;
            if let Ok(pane) = herdr.pane_info(&new_pane_id) {
                reassert_for_pane(env, herdr, stderr, &pane)?;
            }
            Ok(())
        }
        "pane_exited" | "pane_closed" => {
            ...
            StateStore::new(&env.state_dir)
                .transaction(|state| {
                    cleanup_pane(state, &pane_id);
                    Ok(())
                })
                .map_err(|error| error.to_string())
        }
...
fn handle_agent_event(
    ...
    let loaded_config = load_config(&env.config_dir);
    ...
    let visible = snapshot
        .as_ref()
        .is_none_or(|snapshot| snapshot.pane_visible(pane_id));
    let status = event.string("agent_status");
    let store = StateStore::new(&env.state_dir);
    let (assignment, play) = store
        .transaction(|state| {
            ...
            let assignment = assign_sound(state, &identity, &loaded_theme.theme.sounds, now_ms());
            let live_status = (!pane.agent_status.is_empty()).then_some(pane.agent_status.as_str());
            let play = apply_status_observation(
                assignment,
                status.as_deref(),
                live_status,
                visible,
            ...
            Ok((assignment.clone(), play))
        })
        .map_err(|error| error.to_string())?;
    if loaded_config.config.enabled {
        report_assignment(herdr, stderr, &assignment, &loaded_theme.theme)?;
        if play {
...
fn reassert_for_pane(
    env: &PluginEnv,
    herdr: &dyn Herdr,
    stderr: &mut dyn Write,
    pane: &crate::herdr::PaneInfo,
) -> Result<(), String> {
    let config = load_config(&env.config_dir).config;
    if !config.enabled {
        return Ok(());
    }
    let theme = load_theme(&env.plugin_root, &env.config_dir, &config.theme)?.theme;
    ...
    if let Some(assignment) = state
        .assignments
        .iter()
        .find(|assignment| assignment.pane_id == pane.pane_id)
    {
        report_assignment(herdr, stderr, assignment, &theme)?;
    }
    Ok(())
}
...
fn report_assignment(
    ...
    let metadata = Metadata {
        ...
        display_agent: format!("{} · {}", assignment.agent, sound.display_name),
        raw_agent: assignment.agent.clone(),
        ttl_ms: 86_400_000,
    };
...
fn toggle_mute(
    ...
    save_config(&env.config_dir, &config).map_err(|error| error.to_string())?;
    if !config.enabled {
        let state = StateStore::new(&env.state_dir)
            .read()
            .map_err(|error| error.to_string())?
        ...
        for assignment in state.assignments {
            let live_pane_id = match &snapshot {
                Some(snapshot) => {
                ...
                None => assignment.pane_id.as_str(),
            };
            if let Err(error) = herdr.clear_metadata(live_pane_id, &assignment.agent) {
...
fn reshuffle(
    ...
    let assignment = StateStore::new(&env.state_dir)
        .transaction(|state| {
            reshuffle_pane(state, &pane_id, &theme.sounds, now_ms());
            Ok(state
                .assignments
                .iter()
